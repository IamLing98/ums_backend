...
public class SubjectRegistrationController {
    ...
    public ResponseEntity<?> findBy(@PathVariable("termId") String termId, SecurityContextHolder request)
        ...
        return new ResponseEntity<>(subjectRegistrationService.getListSubjectSubmitted(termId, SecurityContextHolder.getContext().getAuthentication().getName()), HttpStatus.OK);
    }
